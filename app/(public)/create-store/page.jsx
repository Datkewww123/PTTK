...
const fetchSellerStatus = async () => {
  ...
  try {
    ...
    if(['approved', 'rejected', 'pending'].includes(data.status)){
      ...
      switch (data.status) {
        case "approved":
        setMessage("Your store has been approved, you now can add products to your store from dashboard")
        setTimeout(() => router.push('/store'), 5000)
        break;
        case "rejected":
        setMessage("Your store has not been approved, contact the admin for more details")
        break;  
         case "pending":
